...
# 2022/4/8
"""
1、注释掉了 def _warning(self):函数的警告提示
2、将np.mean()替换成了np.nanmean()
"""
import copy
import math
import warnings
...
class FeatureExtractor(PipelineStep):
    ...
    def precompute(self,link_path: Union[None, str, Path] = None,precompute_path: Union[None, str, Path] = None,) -> None:
        """
...
class HandcraftedFeatureExtractor(FeatureExtractor):
    ...
    def _extract_features(self, input_image: np.ndarray, instance_map: np.ndarray) -> torch.Tensor:
        ...
        for region_id, region in enumerate(regions):
            ...
            shape_factor = 4 * np.pi * area / ((convex_hull_perimeter+0.000001) ** 2)   # 分母可能出现0，加上一个小数防除0
    ...
    @staticmethod
    def _compute_crowdedness(centroids, k=1000):
        n_centroids = len(centroids)
        if n_centroids < 3:  # 0< 细胞核数 < 3
            mean_crow = np.array([[0]] * n_centroids)
            std_crow = np.array([[0]] * n_centroids)
            return mean_crow, std_crow
        if n_centroids < k:  # 3< 细胞核数 < 10(k的默认值)
            k = n_centroids - 2
            # kth=n_centroids-2 可以直接在这里定义kth值
        ...
        mean_crow = np.reshape(np.nanmean(x, axis=1), newshape=(-1, 1))
...
class InstanceMapPatchDataset(Dataset):
    ...
    def _warning(self):
        ...
        # if np.nanmean(self.patch_overlap) < self.warning_threshold:
        #     pass
            # 警告提示
...
class DeepFeatureExtractor(FeatureExtractor):
    ...
    def __init__(self,architecture: str,patch_size: int,resize_size: int = None,
        stride: int = None,downsample_factor: int = 1,normalizer: Optional[dict] = None,
        batch_size: int = 32,fill_value: int = 255,num_workers: int = 0,verbose: bool = False,
        with_instance_masking: bool = False,**kwargs) -> None:
        """
