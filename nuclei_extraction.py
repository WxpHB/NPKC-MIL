...
#from skimage.morphology import remove_small_objects  有bug，要自定义该函数！！
from skimage.measure import regionprops

from utils.utils import remove_small_objects
...
# ===========       ====================
# ===========GPU占用====================
GPU_DEFAULT_BATCH_SIZE = 14  # default=16，会占用6.8G现存，降为8
CPU_DEFAULT_BATCH_SIZE = 2
# ===========       ====================


DATASET_TO_BOX_URL = {"pannuke": "https://ibm.box.com/shared/static/hrt04i3dcv1ph1veoz8x6g8a72u0uw58.pt",
...
class NucleiExtractor(PipelineStep):
    """Nuclei extraction"""
    def __init__(self,pretrained_data: str = "pannuke",model_path: str = None,batch_size: int = None,**kwargs) -> None:
        ...
        if model_path is None:
            assert pretrained_data in ["pannuke","monusac",], \
    ...
    def _process(  # type: ignore[override]
        self,input_image: np.ndarray,tissue_mask: Optional[np.ndarray] = None,) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def _extract_nuclei(self,input_image: np.ndarray,tissue_mask: Optional[np.ndarray] = None,) -> Tuple[np.ndarray, np.ndarray]:
        ...
        image_loader = DataLoader(image_dataset,shuffle=False,batch_size=self.batch_size,collate_fn=collate)
        pred_map = torch.empty(size=(image_dataset.max_x_coord, image_dataset.max_y_coord, 3),dtype=torch.float32,device=self.device,)
        # save time
    ...
    def precompute(self,link_path: Union[None, str, Path] = None,
                   precompute_path: Union[None, str, Path] = None,) -> None:
...
class ImageToPatchDataset(Dataset):
    ...
    def __init__(self,image: np.ndarray) -> None:
        ...
        """
        self.image = image
        self.dataset_transform = transforms.Compose([transforms.ToTensor(),])
...
def process_np_hv_channels(pred: np.ndarray) -> np.ndarray:
    ...
    proba_map = remove_small_objects(proba_map, min_size=10)  # 有一个Warning
